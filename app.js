...
equiposBase.forEach(eq=>{
  // ✅ Evita duplicados o errores antiguos
...
function render(){
  ...
  for(let eq of equiposBase){
    ...
    // ✅ SOLO 3 LETRAS SI EXISTE, SI NO GENERA (PAN, HIS, CC)
...
// ✅ DASHBOARD (SIN DUPLICADOS + AGRUPADO)
function renderDashboard(){
  ...
  // ✅ mostrar restante si no completa grupo
  if(grupo.length > 0){
    cont.innerHTML += `<div>${grupo.join(" | ")}</div>`;
  }
}
